...
local SuccessorUI = ns.SuccessorUI or {}
...
function SuccessorUI.UpdateApplicantRow(applicantID)
  if not Successor.SuccessorDB or not Successor.SuccessorDB.enabled then
    return
  end
...
function SuccessorUI.ShowApplicantTooltip(name, applicantID, memberIndex)
  ...
  if scoreData.mythicPlusData then
    ...
    GameTooltip:AddDoubleLine('Success Rate:', string.format('%.0f%%', runStats.successRate * 100), 1, 1, 1, 1, 1, 1)
